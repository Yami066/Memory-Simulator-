...
export default function HardwareGrid({ state }) {
  ...
  return (
    <div className={`flex flex-col gap-8 ${s.hardwareTheme}`}>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        {/* CPU */}
        <div
          className={`${s.card} rounded-none flex flex-col min-h-[150px]`}
          style={{
        ...
        >
          ...
          <div
            className="mt-auto rounded-none p-4 text-sm font-medium min-h-[72px] flex items-center justify-center"
            style={{
        ...
        >
          ...
          >
            {faults > 0
              ? <span className="text-center"><span className="font-bold text-lg" style={{ color: '#ff6b6b' }}>{faults}</span> swap{faults !== 1 ? 's' : ''} done</span>
              : <span className="italic text-center" style={{ color: 'var(--win-text-secondary)' }}>Standby</span>
            }
          </div>
        </div>
      </div>
